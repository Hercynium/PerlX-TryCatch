...
Transform this:
  ...
  try {
    ...
    my $wax = wantarray; $wax = $wax ? "list" : defined $wax ? "scalar" : "void";
    print "inside a finally block. context is [$wax]\n";
    my $retval = "returned from a finally block";
    (wantarray ? return split / /, $retval : return $retval) if $return_from_trycatch;
    my $foo = "fell off the end of a finally block";
  }
